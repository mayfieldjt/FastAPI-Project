...
const Result = ({ results, evaluation, onSelectDrug, hasSelection, loading }) => {
  const generateChart = (evaluation) => {
    ...
    const validRanges = ranges.filter((range) => range);

    if (validRanges.length === 2) {
      const missingIndex = ranges.findIndex((range) => !range);
    ...
    const heatmapData = {
      z: ranges.map((range) => range || [null, null]),
    ...
    return { chartData: [heatmapData, observedMarker], minValue, maxValue };
  };
