...
const Result = ({ results, evaluation, onSelectDrug, hasSelection, loading }) => {
  const generateChart = (evaluation) => {
    ...
    // Ensure ranges are arrays and calculate the min and max for the chart
    const normalRange = Array.isArray(normal_level_mg) ? normal_level_mg : [];
    const toxicRange = Array.isArray(toxic_level_mg) ? toxic_level_mg : [];
    const lethalRange = Array.isArray(lethal_level_mg) ? lethal_level_mg : [];
    ...
    const heatmapData = {
      z: [
        normalRange.length > 0 ? normalRange : [minValue],
        toxicRange.length > 0 ? toxicRange : [minValue],
        lethalRange.length > 0 ? lethalRange : [minValue],
      ],
