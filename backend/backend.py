...
def search_drugs(query: str):
    ...
    search_sql = """
        ...
        lethal_level_ug FROM drugs WHERE LOWER(drug_name) LIKE %s
    """
    search_param = f"{query.lower()}%"
    cursor.execute(search_sql, (search_param,))
...
def search(drug: Optional[str] = None):
    """
...
def update_drug(drug_id: int, updated_drug: DrugUpdate):
    ...
    fetch_sql = "SELECT * FROM drugs WHERE id = %s"
    ...
    update_sql = """
        UPDATE drugs SET drug_name = %s, drug_metadata = %s, normal_level_mg = %s, 
        normal_level_ug = %s, toxic_level_mg = %s, toxic_level_ug = %s, 
        lethal_level_mg = %s, lethal_level_ug = %s WHERE id = %s
    """
...
def add_drug(drug: Drug):
    ...
    cursor.execute("""
        ...
        normal_level_ug, toxic_level_mg, toxic_level_ug, lethal_level_mg, 
        lethal_level_ug) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
